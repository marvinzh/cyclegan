...
class CycleGAN():
    ...
    def __init__(self):
        ...
        domain_label = Parameters['domain_label']
        ...
        # Initialize the domain predictor
        self.Dompred = model_cyclegan.build_domain_predictor_keras(outname='Dom_Pred')

        #optimizer_dompred = SGD(lr=Parameters['base_lr'])
        optimizer_dompred = RMSprop(lr=Parameters['base_lr'])
        # optimizer_dompred = Adam(lr=Parameters['base_lr'], beta_1=0.5, beta_2=0.9)

        self.Dompred.compile(
            loss='categorical_crossentropy',
            optimizer=optimizer_dompred,
            metrics=['accuracy']
        )

        # Freeze Dompred during the training of combined model by setting trainable = False
        self.Dompred.trainable = False

        #fake_ivec_b_for_Dompred = self.G_AB(ivec_a)
        #fake_ivec_b_for_Dompred = Input(shape=self.input_shape, name='fake_ivec_b_for_Dompred')
        #fake_ivec_a_for_Dompred = self.G_BA(ivec_b)
        #fake_ivec_a_for_Dompred = Input(shape=self.input_shape, name='fake_ivec_a_for_Dompred')

        domain_prediction_a = self.Dompred(fake_ivec_a)
        domain_prediction_b = self.Dompred(fake_ivec_b)

        # Combine two generators with one domain predictor.
        ...
        self.combined_Gs_Dompred = Model(
            inputs=[ivec_a, ivec_b],
            outputs=[validity_a, validity_b,
                     cycle_ivec_a, cycle_ivec_b,
                     ide_ivec_a, ide_ivec_b,
                     domain_prediction_a, domain_prediction_b]
        )

        self.combined_Gs_Dompred.compile(
            loss=['mse', 'mse',
                  'mae', 'mae',
                  'mae', 'mae',
        ...
        self.D_A.summary()
        self.D_B.summary()
        self.Dompred.summary()
        self.combined_Gs_Dompred.summary()
    ...
    def train(self, epochs):
        ...
        # Load training dataset from the folders
        ...
        # Assume that fake_a's label is [1, 0], fake_b's label is [0, 1]
        domain_label_a = []
        domain_label_b = []

        for i in range(0, batch_size):
            domain_label_a.append([1, 0])

        for j in range(0, batch_size):
            domain_label_b.append([0, 1])

        domain_label_a = np.array(domain_label_a)
        domain_label_a.reshape(batch_size, 2)
        domain_label_b = np.array(domain_label_b)
        domain_label_b.reshape(batch_size, 2)
        ...
        for epoch in range(epochs):
            for batch_i in range(0, num_batch):
                ...
                dB_loss_real = self.D_B.train_on_batch(ivec_B_feed, real)
                dB_loss_fake = self.D_B.train_on_batch(fake_B, fake)

                dA_loss_real = self.D_A.train_on_batch(ivec_A_feed, real)
                ...
                # Train the generators with the domain predictor
                G_loss = self.combined_Gs_Dompred.train_on_batch([ivec_A_feed, ivec_B_feed],
                                                                 [real, real,
                                                                  ivec_A_feed, ivec_B_feed,
                                                                  ivec_A_feed, ivec_B_feed,
                                                                  domain_label_a, domain_label_b])

                # ------------------
                #  Train Domain Predictior
                # ------------------
                fake_B_for_Dompred = self.G_AB.predict(ivec_A_feed)
                fake_A_for_Dompred = self.G_BA.predict(ivec_B_feed)

                self.Dompred.train_on_batch(fake_B_for_Dompred, domain_label_b)
                Dompred_loss = self.Dompred.train_on_batch(fake_A_for_Dompred, domain_label_a)

                elapsed_time = datetime.now() - start_time

                # Plot the progress
                if batch_i % 100 == 0 or batch_i == num_batch:
                    print("[Epoch %d/%d] [Batch %d/%d] [D loss: %f, acc: %3d%%] [G loss: %05f, adv: %05f, cyc: %05f, id: %05f] [Dompred loss: %05f] time: %s " \
                        % (epoch + 1, epochs,
                           batch_i, num_batch,
            ...
            if int(model_num) % 5 == 0:
                model_Gs_Dompred_path = model_path + '/combined_Gs_Dompred_' + model_num + '.h5'
                self.combined_Gs_Dompred.save(model_Gs_Dompred_path)
    ...
    def transform(self, eval_condition="C5", model_use=None, ep_path=None):
        ...
        input_folder_test = './exp/' + eval_condition + '/ivectors_sre10_test_c5/'

        # Load trained Generators model
        # Gs = load_model(model_use)
        Gs = self.combined_Gs_Dompred
        ...
        for k in range(0, 2):
            ...
            for i in range(0, max_data):
                ...
                fake_A_from_b = Gs.get_layer('model_3').predict(test_feed)

                fake_A_from_b = fake_A_from_b.reshape((1, model_cyclegan.IVEC_DIM))
                fake_A_from_b_list = fake_A_from_b.tolist()

                if k == 0:
                    output_folder = './exp/' + eval_condition + '/ivectors_adpt_sre10_test_c5/'
                    data_prep.adpt_ivec2kaldi(fake_A_from_b_list, test_label[i],
                                              arkfilepath=output_folder + 'sre10_test_c5.ark')
                elif k == 1:
                    output_folder = './exp/' + eval_condition + '/ivectors_adpt_sre10_enroll/'
                    data_prep.adpt_ivec2kaldi(fake_A_from_b_list, test_label[i],
                                              arkfilepath=output_folder + 'sre10_enroll.ark')
...
if __name__ == '__main__':
    ...
    if to_train > 0:
        ...
        for num_ep in {'50'}:
            modelpath = './models/1902020221/combined_Gs_Dompred' + num_ep + '.h5'
